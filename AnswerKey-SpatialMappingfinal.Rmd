---
title: "Answer Key: Spatial_Data_and_Mapping"
output:
...
**Data are in "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs". WGS84 is the most commonly used CRS (it's even used by Google Earth). You can use proj4string(), crs(), or st_crs() to check the coordinate system of your spatial data.**
...
**Elements like borders, lines, points, and more won't align. As you can see in the maps below, the borders are slightly different in the two plots (one is more rounded, stretched, etc).** 
...
identical(crs(SpurgePoint),crs(study_area))
identicalCRS(SpurgePoint,study_area)
```
...
**Vector lines still plot to their full extent. So we are seeing the rivers and roads being plotted from beginning to end. We can use crop() to cut off any data outside of our study area.** 
...
**Now all our features are cropped so they don't extend past the study area boundaries. You can compare the length of the roads before and after our crop using the gLength() function.** 
...
**Resolution! Remember, rasters are pixel-based and cannot be scaled infinitely without loss of resolution.**
...
SpurgePoints <- readOGR(dsn = ".", layer = "SpurgePoint")
FremontMRoads <- readOGR(dsn = ".", layer = "FremontMainRoads")
FremontWater <- readOGR(dsn = ".", layer = "WaterwaysFremont")
FremontCo <- readOGR(dsn = ".", layer = "FremontCo")

#crop/mask data
Slope<-mask(Slope,FremontCo)
FremontWater<-crop(FremontWater,FremontCo)
FremontMRoads<-crop(FremontMRoads,FremontCo)
SpurgePoints<-crop(SpurgePoints,FremontCo)

#Fortify all spatial data
SpurgePointsF<-fortify(SpurgePoints)
...
slope.df <- data.frame(map.p)
colnames(slope.df)<-c("Longitude", "Latitude", "slope")  #and make appropriate column headings
slopedf<-fortify(slope.df)

# Last, plot it all with ggplot. (You can use the Elevation code for reference!)
  ggplot(slopedf, aes(y=Latitude, x=Longitude)) +
